...
const TextForm: React.FC<OptionsProps> = ({ site, setSite, componentId, variableId }) => {
    ...
    useEffect(() => {
        if (variable) {
            if (variable.templateParameters === undefined) {
                setSite((prev) => getNewTemplateParams(prev, componentId, variableId, { text: "" }));
            }
        }
    }, [site, variable, componentId, variableId]);

    const onTextChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        setSite((prev) => getNewTemplateParams(prev, componentId, variableId, { text: event.target.value }));
    };
    ...
    return variable?.templateParameters ? (
        <TextField
            id="text"
            label="Text"
            value={variable.templateParameters.text}
